...
void descompacta(TNo *no , char arrPrefixo[], int arrLength, int *i) {
	...
	if((no !=NULL) && (strcmp(x,'0')==0)){
		descompacta(no->esq , arrPrefixo, arrLength, i+1);
		if(no->esq == NULL){
			printf("%c",no->ch);
		}
	}
	
	if((no!=NULL) && (strcmp(x,'1')==0)){
		descompacta(no->dir , arrPrefixo, arrLength, i+1);
		if(no->dir == NULL){
			printf("%c",no->ch);
		}
	}
...
int main(){
	...
	int z=0;
	descompacta(novaRaiz , prefixoRemontado, x, &z);
}
